...
class Features:
    ...
    # def __init__(self, defaultType='discrete 1000000'):
    def __init__(self, defaultType=str):
    ...
    def feature(self, *args, **kargs):
        ...
        def decorate(fn):
            ...
            self.features.append(Dict(name=name, extractor=fn, args=argspec.args, type=type_))
            # print(self.attrs)
            # ja neko neatgriež, tad funkcijas nosaukums globāli satur None

        if len(args) == 1 and hasattr(args[0], '__call__') and not kargs and args[0] != str and args[0] != int:
