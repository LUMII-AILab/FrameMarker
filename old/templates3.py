...
def getFrames(tokens):
    ...
    for frameType in frameTypesWithElementNames:
        ...
        for token in tokens:
            r = function(**frameTargetFeatures(token=token, tokens=tokens))
            if r:
                frames.append(Dict(type=frameType, tokenIndex=token.index, rule=Dict(ok=r[0], cover=r[1]), elements=[]))
...
def getFrameElements(tokens, frames, maxdist=3):
    ...
    for frame in frames:
        for elementName in frameTypesWithElementNames[frame.type]:
            ...
            for token in tokens:
                r = function(**frameElementFeatures(token=token, tokens=tokens, elementName=elementName, frame=frame))
                if r:
                    if token.index == frame.tokenIndex or len(tokens[frame.tokenIndex].distances[token.index]) <= maxdist:
                        frame.elements.append(Dict(tokenIndex=token.index, name=elementName, rule=Dict(ok=r[0], cover=r[1])))
